...
def analyze_news_sentiment_distilroberta(ticker):
    """
    Analyzes the sentiment of news headlines for a given stock ticker using the DistilRoberta model fine-tuned
    for financial news sentiment analysis. The function loads the news data, applies sentiment analysis,
    ...
    Returns:
        pd.DataFrame: A DataFrame containing the original news data along with sentiment analysis results,
                      including positive, neutral, negative scores, and the DistilRoberta Sentiment Score.
    """
    # Load the CSV file containing news data for the specified ticker
    news_data = pd.read_csv(f'news_{ticker}.csv')

    # Load the DistilRoBerta model, tokenizer, and configuration
    MODEL = "mrm8488/distilroberta-finetuned-financial-news-sentiment-analysis"
    tokenizer = AutoTokenizer.from_pretrained(MODEL)
    config = AutoConfig.from_pretrained(MODEL)
    model = AutoModelForSequenceClassification.from_pretrained(MODEL)

    # Function to analyze the sentiment of a headline
    def analyze_sentiment(headline):
        """
        Analyzes the sentiment of a single headline using DistilRoberta.
    ...
    # Concatenate the sentiment score columns with the original DataFrame
    news_data = pd.concat([news_data, sentiment_scores], axis=1)

    # Calculate the DistilRoBERTa Sentiment Score by subtracting the negative score from the positive score
